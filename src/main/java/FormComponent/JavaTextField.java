     ...
     public void setValidatePhoneNumber() {
          ...
          txt.addKeyListener(new KeyAdapter() {
               ...
               public void keyReleased(KeyEvent e) {
                    ...
                    // Get the raw input (remove spaces)
                    String rawText = txt.getText().replaceAll("\\s", "");
                    ...
                    // Adjust caret position based on the added spaces
                    int adjustedCaretPosition = Math.min(caretPosition + (formattedString.length() - rawText.length()), formattedString.length());
                    txt.setCaretPosition(adjustedCaretPosition);
     ...
     public void setValidateNumber() {
          txt.addKeyListener(new KeyListener() {
               ...
               public void keyTyped(KeyEvent e) {
                    ...
                    // Allow digits, backspace, delete, and one dot (.)
                    if (!Character.isDigit(c) && c != KeyEvent.VK_BACK_SPACE && c != KeyEvent.VK_DELETE && c != '.') {
                         e.consume(); // Ignore non-digit characters
                    }
                    ...
                    // Ensure only one decimal point is allowed and it can't be the first character
                    if (c == '.') {
                         // If the current text is empty or contains a dot, prevent input
                         if (currentText.isEmpty() || currentText.contains(".")) {
                              e.consume();
                         }
                    }

                    // Prevent entering two consecutive zeros ("00")
                    if (currentText.equals("0") && c == '0') {
                         e.consume(); // Ignore input if it's "00"
                    }

                    // Prevent leading zeros unless the number is just "0" or "0.<something>"
                    if (currentText.isEmpty() && c == '0') {
                         // Allow the number to start with a zero if it's followed by a dot or any other digit
                         return; // Do nothing, let the zero be typed
                    }

                    // If the current text starts with "0" and the next digit is not a dot or another zero, prevent typing
                    if (currentText.startsWith("0") && currentText.length() == 1 && c != '.' && c != KeyEvent.VK_BACK_SPACE) {
                         e.consume(); // Prevent entering more digits after "0"
                    }
               }
               ...
               @Override
               public void keyReleased(KeyEvent e) {
                    // No action needed on keyReleased, only validation happens on keyTyped
               }
          });
     }
